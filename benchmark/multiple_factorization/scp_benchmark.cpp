...
//
// SCP Benchmark: Computes ordering once, performs symbolic analysis once,
// factorizes once, then loops through RHS vectors for repeated solves.
//
...
#include "utils/scp_prep_benchmark.h"
#include "csv_utils.h"
...
struct CLIArgs
{
    int binary_level = 9;
    int DIM = 1;
    std::string output_csv_address = "/home/behrooz/Desktop/Last_Project/gpu_ordering/output/SCP/scp";
    std::string solver_type   = "CUDSS";
    std::string ordering_type = "DEFAULT";
    std::string patch_type = "rxmesh";
    std::string check_point_address = "/media/behrooz/FarazHard/Last_Project/SCP_benchmark/bunny";
    bool use_gpu = false;

    CLIArgs(int argc, char* argv[])
    {
        CLI::App app{"SCP Benchmark - Single factorization with multiple solves"};
        ...
        app.add_option("-k,--check_point_address", check_point_address, "Folder containing SCP benchmark data");
        app.add_option("-d,--DIM", DIM, "Dimension of the mesh (3 for SCP)");
...
int main(int argc, char* argv[])
{
    ...
    if (hessian_address.empty()) {
        spdlog::error("No Hessian file found. Exiting.");
        return 1;
    }
    if (rhs_addresses.empty()) {
        spdlog::error("No RHS files found. Exiting.");
        return 1;
    }
    ...
    // ========== Load the Hessian matrix (single matrix for SCP) ==========
    Eigen::SparseMatrix<double> hessian;
    Eigen::loadMarket(hessian, hessian_address);
    ...
    parth_compressor.setMatrix(hessian.rows(), hessian.outerIndexPtr(), hessian.innerIndexPtr(), args.DIM);
    ...
    if (!matrix_perm.empty()) {
        factor_nnz = RXMESH_SOLVER::get_factor_nnz(hessian.outerIndexPtr(),
                                                   ...
                                                   hessian.nonZeros(),
                                                   matrix_perm);
        spdlog::info("Factor NNZ ratio: {:.4f}", factor_nnz * 1.0 / hessian.nonZeros());
        if (ordering != nullptr) {
    ...
    // ========== Factorize once (SCP uses single Hessian) ==========
    ...
    // ========== Prepare full Hessian for residual computation ==========
    ...
    for (size_t iter = 0; iter < rhs_addresses.size(); iter++) {
        spdlog::info("--- Iteration {} ---", iter);

        // Load RHS vector
        Eigen::VectorXd rhs;
        Eigen::loadMarketVector(rhs, rhs_addresses[iter]);

        // Result vector
        Eigen::VectorXd result;

        // Solve (factorization already done)
    ...
    spdlog::info("=== SCP Benchmark Complete ===");
