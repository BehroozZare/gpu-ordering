...
Computes and visualizes speedup of PATCH_ORDERING (nd_levels=9) over DEFAULT
ordering for CUDSS solver.
"""
...
def main():
    ...
    data_file = script_dir / "data" / "end_to_end.csv"
    ...
    default_df = df[
        ...
        (df["solver_type"] == "CUDSS")
    ].copy()
    print(f"DEFAULT entries (CUDSS): {len(default_df)}")
    
    # Filter for PATCH_ORDERING with nd_levels=9 and CUDSS solver_type
    patch_df = df[
        (df["ordering_type"] == "PATCH_ORDERING") & 
        (df["solver_type"] == "CUDSS") &
        (df["nd_levels"] == 9)
    ].copy()
    print(f"PATCH_ORDERING (nd_levels=9, CUDSS) entries: {len(patch_df)}")
    
    # Compute total runtime for DEFAULT
    # Treat ordering_time = -1 as 0
    default_df["total_time"] = (
        default_df["ordering_time"].clip(lower=0) +
    ...
    print(f"Matched mesh pairs: {len(merged_df)}")
